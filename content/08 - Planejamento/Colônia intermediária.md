---
tipo: guia
status: publicado
ultima_revisao: 2026-07-16
tags: [minecolonies, planejamento, colônia-intermediária, lote-9a]
---

# Colônia intermediária

A fase intermediária transforma uma fundação estável em uma rede produtiva. O foco muda de sobrevivência para pesquisa, especialização e logística.

## Frentes de expansão

### Pesquisa e cidadãos

- construa e melhore a University;
- use School e Library para desenvolver atributos;
- mantenha Hospital e Graveyard compatíveis com o tamanho da população;
- acompanhe felicidade, moradia e tempo de deslocamento.

### Produção

- escolha oficinas pelos pedidos reais do Builder;
- ensine receitas recorrentes;
- conecte madeira, pedra e metal à logística;
- amplie alimentação com Chef’s Kitchen, Bakery e criação animal.

### Defesa

- cubra acessos com Guard Towers;
- estabeleça patrulhas;
- prepare Barracks e centros de treinamento;
- mantenha equipamento e alimento para guardas.

## Critério para avançar

- University mantém uma fila de pesquisas útil;
- principais materiais de construção são produzidos pela colônia;
- Warehouse não permanece congestionado;
- invasões são contidas sem perdas recorrentes;
- alimento possui variedade e margem de estoque;
- oficinas críticas funcionam sem pedidos manuais frequentes.

## Método de expansão

```mermaid
flowchart LR
    A["Identificar gargalo"] --> B["Escolher pesquisa ou construção"]
    B --> C["Garantir entradas"]
    C --> D["Ensinar receitas"]
    D --> E["Conectar Courier"]
    E --> F["Medir resultado"]
    F --> A
```

> [!NOTE] Análise do Vault
> Construa uma oficina quando ela resolver um gargalo recorrente. Erguer todas ao mesmo tempo aumenta população, pedidos e deslocamentos sem garantir produtividade.

## Próxima etapa

[[content/08 - Planejamento/Colônia avançada]]

## Fontes

- [Research — Wiki oficial do MineColonies](https://minecolonies.com/wiki/systems/research/)
- [Request System — Wiki oficial do MineColonies](https://minecolonies.com/wiki/systems/request/)
