---
tipo: sistema
status: publicado
ultima_revisao: 2026-07-21
tags: [minecolonies, administração, contratação, lote-4]
---

# Contratação automática

## Controle global e local

A Prefeitura possui **Auto Worker Hiring**. Quando ativado, cidadãos ocupam vagas abertas somente se a cabana também estiver configurada para contratação automática.

| Prefeitura | Cabana | Resultado |
|---|---|---|
| Automático | Automático | O jogo preenche a vaga |
| Automático | Manual | O jogador escolhe nessa cabana |
| Manual | Qualquer modo | O jogador administra contratações |

## Quando usar

- **Automático:** fundação, muitas vagas equivalentes ou pouca necessidade de otimização.
- **Manual:** Researchers, Teachers, Doctors, artesãos especializados e postos em que habilidades influenciam fortemente o resultado.

## Processo manual

1. Desative o automático globalmente ou na cabana.
2. Abra **Manage Workers**.
3. Compare as habilidades relevantes.
4. Considere a distância entre Residência e trabalho.
5. Confirme a atribuição e acompanhe o primeiro ciclo.

## Moradia

**Auto Citizen Housing** é uma configuração separada. Desativá-la permite aproximar cidadãos do trabalho, mas exige manutenção sempre que a população mudar.

## Fontes

- [Town Hall — Wiki oficial do MineColonies](https://minecolonies.com/wiki/buildings/townhall/)
