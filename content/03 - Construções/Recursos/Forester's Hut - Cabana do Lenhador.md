---
tipo: construção
nome: Forester's Hut
nome_pt: Cabana do Lenhador
aliases: [Lumberjack's Hut]
categoria: Recursos
versao_minecraft: 1.20.1
versao_minecolonies: 1.1.1259
tipo_versao: prerelease
estilo: Medieval Dark Oak
nivel_maximo: 5
trabalhador: Forester
status: em_revisao
revisao_pendente: [progressao]
ultima_revisao: 2026-07-21
tags: [minecolonies, construção, madeira, lote-2]
---

# Forester's Hut — Cabana do Lenhador

<!-- ficha-visual: bloco -->
<img src="../../../assets/itens/blocos-de-construcao/blockhutlumberjack/principal.png" alt="Bloco da construção Cabana do Lenhador" width="96" align="right">

## Galeria — Medieval Dark Oak

| Vista frontal | Vista traseira |
|---|---|
| ![[assets/construcoes/medieval-dark-oak/fundamentals/lumberjack/front.jpg]] | ![[assets/construcoes/medieval-dark-oak/fundamentals/lumberjack/back.jpg]] |

> [!INFO] Variante disponível
> O acervo também contém `fundamentals/altlumberjack`.

## Visão geral

O Lenhador corta árvores, coleta madeira e pode replantar mudas. Por padrão, procura árvores em uma área ampla ao redor do próprio trabalhador; zonas de corte oferecem controle mais seguro.


## Interface do bloco

<!-- galeria-interface -->
### Galeria da interface

| Principal | Receitas de fabricação |
|---|---|
| ![Principal — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/principal.png) | ![Receitas de fabricação — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/receitas-de-fabricacao.png) |

| Mudas | Configurações |
|---|---|
| ![Mudas — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/mudas.png) | ![Configurações — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/configuracoes.png) |

| Estoque mínimo | Tarefas |
|---|---|
| ![Estoque mínimo — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/estoque.png) | ![Tarefas — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/tarefas.png) |

| Zona de trabalho |
|---|
| ![Zona de trabalho — Cabana do Lenhador](../../../assets/interfaces/construcoes/lumberjack/zona-de-trabalho.png) |

## Configurações importantes

- **Lista de mudas permitidas** (*Sapling whitelist*): define quais mudas podem ser plantadas.
- **Replantar** (*Replant*): ativa o replantio após o corte.
- **Área de corte** (*Chopping zone*): restringe a atividade a uma área marcada.
- **Estoque mínimo** (*Minimum Stock*): mantém ferramentas e materiais essenciais disponíveis.
- **Receitas** (*Recipes*): permite ensinar transformações úteis quando suportadas.

## Proteção de árvores decorativas

Árvores que fazem parte do esquema da cabana não são tratadas como alvo. Uma árvore com paralelepípedo diretamente abaixo também pode ser protegida. Para planejamento previsível, prefira delimitar uma zona de corte.

## Posicionamento

- reserve uma floresta renovável perto da cabana;
- mantenha distância de parques e árvores ornamentais;
- conecte a produção ao Armazém;
- use espaço suficiente para replantio e crescimento.

## Profissão

Consulte [[content/04 - Profissões/Forester - Lenhador]].

## Fontes

- [Forester's Hut — Wiki oficial do MineColonies](https://minecolonies.com/wiki/buildings/lumberjack/)
